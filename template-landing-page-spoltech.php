<?php
	/*
 * Template name: Landing Page SpolTech
 */
...
<header class="header landing-page-header-spoltech <?php $tooltip = get_field('header-tooltip-toggle'); if (is_tooltip_enabled($obrazek)) echo 'tooltip-enabled'; ?>"
...
<?php 
    $obrazek = get_field('header-image');
	$obrazekMobile = get_field('header-image-mobile');
?>

<header class="header landing-page-header-spoltech visible-lg visible-md <?php $tooltip = get_field('header-tooltip-toggle'); if (is_tooltip_enabled($obrazek)) echo 'tooltip-enabled'; ?>"
    style="background-image: url('<?php echo $obrazek['url'] ?>')">
...
<header class="header landing-page-header-spoltech hidden-lg hidden-md <?php $tooltip = get_field('header-tooltip-toggle'); if (is_tooltip_enabled($obrazekMobile)) echo 'tooltip-enabled'; ?>"
    style="background-image: url('<?php echo $obrazekMobile['url'] ?>')">

    <div class="content">
        <div class="content-box">
		<div class="spoltech-header">
			<h1 class="spoltech-header-main">Przyszłość potrzebuje przyjaciół</h1>
        	</div>            
	</div>

    </div>
    <?php cc_tooltip($obrazekMobile) ?>
</header>
