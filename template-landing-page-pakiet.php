<?php
	/*
 * Template name: Landing Page Pakiet
 */
...
<?php
    ...
    $kolor_tla = get_field('kolor_tla_zdjecia_glownego');
    $obrazek_contain = get_field('zdjecie_glowne_zawsze_w_calosci');
?>

<header class="header landing-page-header <?php $tooltip = get_field('header-tooltip-toggle'); if (is_tooltip_enabled($obrazek)) echo 'tooltip-enabled'; ?>"
    style="
        background-image: url('<?php echo $obrazek['url'] ?>');
        background-color: <?php echo $kolor_tla; ?>;
        <?php if ($obrazek_contain) { ?>
    ...
    <div class="container">
        <div class="row">
            <div class="col-md-12 col-sm-12 hidden-xs">
                <ul id="content-nav-ul" class="nav-pakiet">
                </ul>
            </div>
        </div>
    </div>
